...
> Previous: [`A1`](./a1.md). Next: `B1`, in the CLI repo — it consumes what this
> slice publishes.
>
> **Blocked on an access conversation, not on code.** See "The blocker" below.
>
> **Status: implemented, on `zibby`** — not on `main`, not published. The three packages exist under
> `packages/*` and `linter_review_tool/profiles.js` assembles `RELIABLE_RULES` from them, but the npm
> publish itself is still blocked on the access conversation below.
...
Exporting it also makes [`A3`](./a3.md)'s equality test meaningful — a test that
...
packages, never restated locally, or [`A3`](./a3.md)'s equality test compares a copy
...
gap, guarded by [`A3`](./a3.md)'s conformance corpus rather than closed by a package.
...
- v1's rule set is [`A0`](./a0.md)'s frozen baseline, including whatever
  `fix/large-file-inline-comments` decided about `max-lines`. For that to be unambiguous, **A0's
  snapshot must be taken with [`A1`](./a1.md)'s two intentional deltas already
...
conversation, and it should start before [`A1`](./a1.md) finishes so it is not the
