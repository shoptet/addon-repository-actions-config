...
> Next: [`A1`](./a1.md).
>
> **Status: done, retroactively.** This slice was supposed to run before `A1`/`A2` but did not; it ran
> after both, against `zibby`, as unit U3 of the MVP-completion run. The frozen baseline it should
> have produced beforehand is now written down after the fact in
> [`a0-baseline.md`](./a0-baseline.md), including what that ordering cost.
