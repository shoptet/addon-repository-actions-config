...
>
> **Status: implemented, on `zibby`** — not on `main`, not published. The rule-set equality test
> (`linter_review_tool/test/rule-equality.js`) and the discovery conformance corpus
> (`packages/addon-lint-conformance` + `linter_review_tool/test/conformance.js`) both run as part of
> `yarn test`, wired into `.github/workflows/selftest.yml`. The corpus lives in its own fourth
> package rather than either runner's own tree — see `packages/addon-lint-conformance/README.md` for
> the manifest schema the CLI repo's `B4` slice consumes it against. Proven by perturbation: reverting
> A1's `cwd`/basePath fix (`linters/eslint-linter.js`'s `commonAncestorDir(files)` back to a hardcoded
> `cwd: ROOT`) turns 6 of the corpus's 11 shapes red, including the literal
> `"File ignored because outside of base path."` message the `outside-tree-still-blocks` shape's
> `forbidMessagePattern` exists to catch. The equality test was proven against all four divergence
> classes from this doc's own list (a rule enabled in shared config but missing from the effective
> set; a severity flip; a rule configured but absent from `RELIABLE_RULES`; the reverse orphan case),
> each induced, observed failing, then reverted.
