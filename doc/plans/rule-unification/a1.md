...
> Previous: [`A0`](./a0.md). Next: [`A2`](./a2.md).
>
> **Status: implemented, on `zibby`** — not on `main`, not published. Everything below was measured by performing the port on a scratch copy of
...
nothing). Since [`A2`](./a2.md) publishes this config for a consumer on ESLint 10,
...
- `parse5` `^6.0.1` → `^7.3.0` may ride along or wait for [`A2`](./a2.md). Measured
...
- **Pin `node-version` explicitly** (`"22.13"` or `"24"`) rather than `"22"`. This is *not* because
  ...
  [`A2`](./a2.md) publishes: ESLint 10's floor is
...
- `node test/selftest.js` green, with exactly the two intended `expected.json`/config changes above
  and no others. The frozen baseline from [`A0`](./a0.md) is the comparison.
