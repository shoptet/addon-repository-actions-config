...
| `findings` | `{ ruleId, severity, path }[]` | Findings that MUST be present — "at least these": each listed entry must match some reported finding on all three of `ruleId`, `severity` and `path`, while extra, unlisted findings are allowed. So `"findings": []` asserts nothing (it is documentation that no finding is expected, not a check); to assert "no findings on path X", use `findingsExcludePaths` (or `skippedDirs`, which already includes "nothing under this directory produced a finding"). `severity` is `blocker`/`recommend`, this corpus's two-level scale — map a runner's own severity vocabulary onto it the same way `linter_review_tool/review.js`'s `toRdjson()` does: error-equivalent → `blocker`, everything else reportable → `recommend`. |
